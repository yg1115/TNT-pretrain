...
parser.add_argument('data', metavar='DIR',default='./img',
...
parser.add_argument('--sched', default='cosine', type=str, metavar='SCHEDULER', # step
...
#zz add cifar10 dataloader
import torch
from torch.utils.data import DataLoader
import torchvision.datasets as dsets
import numpy as np
batch_size = 128
train_sets = dsets.CIFAR10(root='./img/',  # 选择数据的根目录
                           train=True,  # 选择训练集
                           transform=torchvision.transforms.Compose([torchvision.transforms.ToTensor(),torchvision.transforms.Resize(224)]),  #resize
                           download=False)  # 从网络上下载图片

test_sets = dsets.CIFAR10(root='./img/',  # 选择数据的根目录
                          train=False,  # 选择测试集
                          transform=torchvision.transforms.Compose([torchvision.transforms.ToTensor(),torchvision.transforms.Resize(224)]),
                          download=False)  # 从网络上下载图片
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            output = model(input)
            # target = make_one_hot(target,1000)
            #zz 对cifar数据集标签进行处理 转为onehot格式
            target = torch.nn.functional.one_hot(target, num_classes=1000)
            # print('aa',output.shape,target.shape)
